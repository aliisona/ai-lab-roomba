...
# Name(s): Alisona Le and Vedic Patel
    ...
    def search(self, 
        ...
        - Repeat while there are still StateNodes in the frontier:
            ...
            3*) Call gui_callback_fn, passing it the dequeued StateNode. If it returns True, 
                end the search (the user has terminated early) done
            4) Extend the dequeued state by enqueueing all its neighboring states. 
                - Implement the "no backtracking" optimization: do not enqueue parent states  done
                - Pass the cutoff parameter to enqueue. 
                - Update self.total_extends and self.total_enqueues appropriately done 
        - If the search ends because the frontier is empty or gui_callback_fn ended the search
        early, return None. done 
        ...
        while self.frontier: #while the froniter still has states (self.frontier = True when != 0)
            ...
            for newState in state.get_all_actions():
                ...
                if newState != state.parent: #no backtracking
